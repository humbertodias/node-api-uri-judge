...
app.get('/uri', function (req, res) {
    ...
    for (var value in students){
        ...
        student["tried"] = students[value].tried;
        student["submissions"] = students[value].submissions;
                    ...
                    if (my_browser.success) {
                        ...
                        if (!(typeof $ === 'undefined')) {
                            table = $('#element tr').each(function () {
                                ...
                                if (cellText.text().trim() == "SENAC") {
                                    rank_ufrrj = $(this).children('td').eq(0).text().trim();
                                    points_ufrrj = $(this).children('td').eq(3).text().trim();
                                    total_students = $(this).children('td').eq(4).text().trim();
                ...
                elem.each(function(i){
                	if(i>0 && i<=top){//skips i==0 -> table header
                        var student = {
                            ...
                            points: $(this).find(".small").eq(0).text().trim(),
                            tried: $(this).find(".small").eq(1).text().trim(),
                            submissions: $(this).find(".small").eq(2).text().trim(),
